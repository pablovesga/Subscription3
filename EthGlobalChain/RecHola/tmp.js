...
var onCronTrigger = (runtime2) => {
  runtime2.log("Workflow triggered - Reading agreement data and executing payment");
  ...
  runtime2.log(`Reading agreement ID: ${agreementId}`);
  ...
  runtime2.log(`Agreement Data:`);
  runtime2.log(`  Sender: ${result.sender}`);
  runtime2.log(`  Receiver: ${result.receiver}`);
  runtime2.log(`  Amount: ${result.amount}`);
  runtime2.log(`  Interval: ${result.interval} seconds`);
  runtime2.log(`  Next Payment: ${result.nextPayment} (timestamp)`);
  runtime2.log(`  Installments Paid: ${result.installmentsPaid}`);
  runtime2.log(`  Total Installments: ${result.totalInstallments}`);
  runtime2.log(`  Is Active: ${result.isActive}`);
  const txHash = executePayInstallment(runtime2, network248.chainSelector.selector, evmConfig, agreementId);
  return {
    agreementId,
...
function executePayInstallment(runtime2, chainSelector, evmConfig, agreementId) {
  ...
  const reportData = encodeAbiParameters(parseAbiParameters("uint256 id"), [agreementId]);
  ...
  runtime2.log("Waiting for transaction confirmation");
  const txHash = bytesToHex(writeReportResult.txHash || new Uint8Array(32));
  runtime2.log(`Payment transaction succeeded: ${txHash}`);
...
var initWorkflow = (config) => {
  ...
  return [
    cre.handler(cron.trigger({ schedule: "*/30 * * * * *" }), onCronTrigger)
  ];
};
async function main() {
